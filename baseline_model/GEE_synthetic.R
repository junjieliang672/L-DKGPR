...
lambda <- 0.005  # 0.2 # this is chosen by the cv
formula <- "label~.-iid+0"
...
if(file_base == 'longitudinal'){
  dim(myfit1$working.correlation)
  corr <- myfit1$working.correlation
  alpha <- corr[3]/corr[2]
  write.mat(list('alpha'=alpha),paste0(loc,'../synthetic_corr/','gee_',file))
}else{
  myfit1 <- PGEE(formula = formula, id = iid, data = allData, na.action = NULL, 
                 family = family, corstr = "unstructured", scale.fix = F, beta_int = rep(0,ncol(train)-2),
                 lambda = lambda, eps = 10^-3, maxiter = 30, 
                 tol = 10^-3, silent = T)
  
  dim(myfit1$working.correlation)
  corr <- myfit1$working.correlation
  write.mat(list('corr'=as.matrix(corr)),paste0(loc,'../synthetic_corr/','gee_',file))
}
